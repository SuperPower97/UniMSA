...
from .basic_layers import Transformer, CrossTransformer, HhyperLearningEncoder, GradientReversalLayer, fuse_nig, CrossBipolarAttention
from .bert import BertTextEncoder
from einops import rearrange, repeat


class UniMSA(nn.Module):
    def __init__(self, args):
        super(UniMSA, self).__init__()
        ...
        self.dmml = nn.ModuleList([
            ...
            CrossBipolarAttention(heads=args['model']['dmml']['cross_bipolar_attention']['heads'],
                                  hidden_dim=args['model']['dmml']['cross_bipolar_attention']['hidden_dim']),

        ])
        self.nig_evidence = nn.ModuleList([
            nn.Linear(args['model']['nig_evidence']['nig_mlp1']['input_dim1'], args['model']['nig_evidence']['nig_mlp1']['output_dim1']),
            nn.Linear(args['model']['nig_evidence']['nig_mlp2']['input_dim2'], args['model']['nig_evidence']['nig_mlp2']['output_dim2'])
        ])
    # for output mu, v, alpha, beta
    def evidence_para(self, x):
        return Func.softplus(x)     # [0, 1]

    def evidence(self, x):
        return Func.tanh(x) * 3     # [-3, 3]
    
    def trans(self, gamma, logv, logalpha, logbeta):
        v = self.evidence_para(logv)
        alpha = self.evidence_para(logalpha) + 1
        beta = self.evidence_para(logbeta)
        gamma = self.evidence(gamma)
    ...
    def forward(self, complete_input, incomplete_input):
        ...
        # Proxy Dominant Feature Generation.
        ...
        # Bipolar Attention 
        feat1, pos, neg = self.dmml[2](h_hyper, h_d_list[-1])
        # Uncertainty Estimator
        output1 = self.nig_evidence[0](torch.mean(feat1, dim=1)) 
        gamma_g, v_g, alpha_g, beta_g = torch.split(output1, 1, dim=-1)
        gamma_g, v_g, alpha_g, beta_g = self.trans(gamma_g, v_g, alpha_g, beta_g)
        feat2 = h_hyper
        output2 = self.nig_evidence[1](torch.mean(feat2, dim=1)) 
        gamma_a, v_a, alpha_a, beta_a = torch.split(output2, 1, dim=-1)
        gamma_a, v_a, alpha_a, beta_a = self.trans(gamma_a, v_a, alpha_a, beta_a)
        ...
        return {'sentiment_preds': 
                    {'gamma_g': gamma_g, 'v_g': v_g, 
                     'alpha_g':alpha_g, 'beta_g':beta_g, 
                     'gamma_a':gamma_a, 'v_a':v_a, 
