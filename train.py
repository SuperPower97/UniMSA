...
def main():
    ...
    ckpt_root = os.path.join('ckpt', args['dataset']['datasetName'], f'ablation_wo_NIG')
    # ipdb.set_trace()
...
def train(model, train_loader, optimizer, loss_fn, epoch, metrics):
    ...
    for cur_iter, data in enumerate(tqdm(train_loader, desc="Training Progress", unit="batch")):
        ...
        loss, y_pred_f = loss_fn(out, label)
        loss['loss'].backward()
        optimizer.step()
        optimizer.zero_grad()
